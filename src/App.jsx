...
const content = {
  asymptotic: () => (
    <div>
      ...
      <Table
        headers={["Notation", "Meaning", "Formal Definition", "Example"]}
        rows={[
          ["O(f(n))", "Upper bound (worst case)", "∃ c, n₀ : T(n) ≤ c·f(n) ∀ n ≥ n₀", "3n² + 5n = O(n²)"],
          ["Ω(f(n))", "Lower bound (best case)", "∃ c, n₀ : T(n) ≥ c·f(n) ∀ n ≥ n₀", "n² + 1 = Ω(n)"],
          ["Θ(f(n))", "Tight bound", "Both O(f(n)) and Ω(f(n))", "2n² + 3 = Θ(n²)"],
        ]}
      />
...
C(x) = 3 + 17x + 22x² + 8x³  →  C = [3, 17, 22, 8]`}</Code>
      ...
      <Table
        headers={["Representation", "Storage", "Evaluation at x", "Addition", "Multiplication"]}
        rows={[
          ["Coefficient form", "[a₀, a₁, ..., aₙ]", "O(n)", "O(n)", "O(n²) naive"],
          ["Point-value form", "[(x₀,y₀),...,(xₙ,yₙ)]", "O(1) if xᵢ=xⱼ", "O(n)", "O(n) pointwise!"],
        ]}
      />

      <Callout title="The Key Insight for FFT" color="#6366f1">
        In point-value form, multiplication is trivially O(n) — just multiply y-values pointwise. The expensive part is converting between the two forms. FFT does this conversion in O(n log n) using the special structure of roots of unity as evaluation points.
      </Callout>
